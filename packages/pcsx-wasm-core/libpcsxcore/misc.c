...
int GetCdromFile(u8 *mdir, u8 *time, s8 *filename) {
	...
	i = 0;
	while (i < 4096) {
		//printf("GetCdromFile i=%d\n", i);
		dir = (struct iso_directory_record*) &mdir[i];
		//showdir(dir);
		if (dir->length[0] == 0) {
			return -1;
		}
		//printf("GetCdromFile length[0]=%d, flags=%x\n", dir->length[0], dir->flags[0]);
		i += dir->length[0];
		...
		if (dir->flags[0] & 0x2) { // it's a dir
			...
			if (!strnicmp((char *)&dir->name[0], filename, strlen(filename))) {
				//printf("GetCdromFile get\n");
				mmssdd(dir->extent, (char *)time);
				//for(int ii=0;ii<4;ii++) printf("%02x ", time[ii]);
				//printf("\n");
				break;
			}
		}
...
int CheckCdrom() {
	...
	for(int ii=0;ii<84;ii++) printf("%02x ", buf[ii]);
	printf("\n");
	...
	for(int ii=0;ii<40;ii++) printf("%02x ", mdir[ii]);
	printf("\n");
	if (GetCdromFile(mdir, time, "SYSTEM.CNF;1") != -1) {
		...
		if (GetCdromFile(mdir, time, exename) == -1) {
			sscanf((char *)buf + 12, "BOOT = cdrom:%256s", exename);
			if (GetCdromFile(mdir, time, exename) == -1) {
				char *ptr = strstr((s8 *)buf + 12, "cdrom:");			// possibly the executable is in some subdir
				if (ptr != NULL) {
					ptr += 6;
					while (*ptr == '\\' || *ptr == '/') ptr++;
					strncpy(exename, ptr, 255);
